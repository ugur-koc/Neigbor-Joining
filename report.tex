...
Neighbor-joining algorithm is deterministic, i,e, given a distance matrix it will always produce the same phylogenetic tree. Therefore computing accurate phylogenetic trees highly depend on the model of evaluation, i,e, distance matrix.

In this project we tried three different ways to generate the distance matrix; 1) we used the \textit{distmat} -which computes distance matrix for already aligned sequences- program from emboss package\footnote{\url{http://emboss.sourceforge.net/apps/release/6.6/emboss/apps/distmat.html}} \cite{rice2000emboss}. Emboss has \textit{distmat} program, . We wrote a wrapper script in perl to call this program and then post process it's output in order to put it in the format of input distance matrix of our NJ implementation (see \textit{aligntodist.pl} script in source code).
...
In all experiments, we used a dataset which is consist of 85 protein sequences of fully sequenced species representing all major Eukaryotic lineages. We took this dataset and the corresponding phylogenetic tree from ~\cite{khafif2014identification}\footnote{See Additional File 5 and 6 \url{http://www.biomedcentral.com/1471-2105/15/222}}. The longest sequence in this set has 125 amino acids and the phylogenetic tree.
